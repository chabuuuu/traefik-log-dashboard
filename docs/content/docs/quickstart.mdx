...
Geographic IP lookup is enabled in the dashboard using a hybrid resolver:

- Optional local MaxMind MMDB (`GEOIP_LOCAL_DB_PATH`)
- HTTP provider fallback (`GEOIP_PROVIDER_URLS` or `GEOIP_PROVIDER_BASE_URL`)

No extra setup is required for HTTP-only mode. For higher reliability, mount a local MMDB file and set `GEOIP_LOCAL_DB_PATH`.
