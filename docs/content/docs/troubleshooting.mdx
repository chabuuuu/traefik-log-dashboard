...
1. **Check lookup health**:
   ```bash
   curl http://localhost:3000/api/location/status
   ```
   Confirm `enabled=true`, and either `local_db_loaded=true` or at least one provider in `providers` with `available=true`.

2. **Run a direct lookup test**:
   ```bash
   curl -s -X POST http://localhost:3000/api/location/lookup \
     -H 'Content-Type: application/json' \
     -d '{"ips":["8.8.8.8","1.1.1.1"]}' | jq .
   ```
   If responses are all `"country":"Unknown"`, inspect provider errors/cooldowns from `/api/location/status`.

3. **Verify logs are geo-eligible**:
...
- Provider-side blocks (e.g. HTTP 403) are surfaced in `/api/location/status` (`providers[].last_error` / `cooldown_until`).
