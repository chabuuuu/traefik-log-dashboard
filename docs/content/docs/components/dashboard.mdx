...
- **GeoIP**: Hybrid resolver (optional local MMDB + HTTP provider fallback)
- **Icons**: Lucide React
- **Language**: TypeScript
...
- Optional local MMDB lookup via `GEOIP_LOCAL_DB_PATH`
- HTTP fallback providers via `GEOIP_PROVIDER_URLS` (or `GEOIP_PROVIDER_BASE_URL`)
- No local DB required for basic functionality
